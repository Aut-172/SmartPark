...
import com.demo.smartpark.order.mapper.ParkingOrderMapper;
...
public class ParkingOrderServiceImpl extends ServiceImpl<ParkingOrderMapper, ParkingOrder> implements IParkingOrderService {
    ...
    public Long createOrder(Long userId, OrderCreateRequest request) {
        // 1. 检查停车场是否存在且有空余车位
        ParkingLot lot = parkingLotService.getById(request.getLotId());
        if (lot == null) {
        ...
        boolean success = parkingLotService.lambdaUpdate()
    ...
    public ParkingOrder completeOrder(Long userId, OrderUpdateRequest request) {
        ...
        // 3. 查询关联停车场获取费率
        ParkingLot lot = parkingLotService.getById(order.getLotId());
        if (lot == null) {
